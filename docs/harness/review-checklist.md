...
> 二审 agent 必读。配合 `.claude/commands/harness-second-review.md` 使用。
> 二审只读 diff，不改文件，不替主开发补代码，不把 P0 降级。
...
如果是独立上下文 + 只读 diff 的二审，使用以下精简流程：

```bash
cat docs/audit/requirements-confirmation.md
cat docs/testing.md
git status --short
git diff --stat
git diff
...
只审查当前 diff，检查：Auth bypass / 角色权限 / ownerUserId 隔离 / MaterialCard 私有性 / ArticleFavorite / UserContentState / SyncRecord / IMA 同步泄露 / AI & IMA env fallback / 缺失 route test / 缺失 A/B 测试 / 不合理 skip / 越界改动。

每个 finding 必须包含文件路径、风险、期望修复。
