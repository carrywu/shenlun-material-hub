# P1: we-mp-rss 权限收紧 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 we-mp-rss 用户级配置接口与设置入口从 VERIFIED_USER 收紧为 ADMIN-only，普通（非管理员）用户完全无法配置、调用或看到 we-mp-rss。
...
**Scope（本 plan 覆盖、不覆盖）：**
- ✅ 覆盖：5 个 settings/we-mp-rss API handler 权限 + 设置页入口 + integrations 页守卫 + 单测 + e2e
- ❌ 不覆盖：`/api/integrations/we-mp-rss/**`（已经是 `requireAdmin`，无需改）、管理员后台 `/admin/integrations/we-mp-rss` 页面（保持不动）、其他 PR 的内容
...
| 文件 | 责任 | 本 plan 操作 |
|---|---|---|
| `src/app/api/settings/integrations/we-mp-rss/route.ts` | 用户级 we-mp-rss 配置（GET/POST/DELETE/PUT） | 改 4 个 handler 的权限校验 |
| `src/app/api/settings/integrations/we-mp-rss/test/route.ts` | 测试连接（POST） | 改权限校验 |
| `src/app/api/settings/integrations/we-mp-rss/__tests__/route.test.ts` | 主路由单测 | 新建（覆盖 4 个 handler 的 ADMIN/VERIFIED_USER/USER/匿名 4 种角色） |
| `src/app/api/settings/integrations/we-mp-rss/test/__tests__/route.test.ts` | test 子路由单测 | 新建 |
| `src/app/settings/page.tsx` | 设置首页（外部集成入口卡片） | 入口条件 `isVerifiedUser` → `isAdmin` |
| `src/app/settings/integrations/page.tsx` | VERIFIED_USER 配置页（含 we-mp-rss 表单） | 加客户端 admin 守卫 |
| `e2e/api-security.spec.ts` | API 安全 e2e | 追加 we-mp-rss 接口的权限用例 |
| `docs/handoff/P1-we-mp-rss-permission-handoff.md` | 交接文档 | 新建 |
...
**Files:**
- Create: `src/app/api/settings/integrations/we-mp-rss/__tests__/route.test.ts`
...
vi.mock("@/services/integrations/we-mp-rss-api", () => ({
  checkHealth: vi.fn(),
}));
...
function makeRequest(
  ...
  return new NextRequest(
    "http://localhost/api/settings/integrations/we-mp-rss",
    init
  );
}

describe("we-mp-rss settings route — permission lockdown", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("GET: ADMIN → 调用 db 查询（200）", async () => {
    ...
    expect(mocks.findUnique).toHaveBeenCalledWith({
      where: {
        userId_provider: { userId: "admin-id", provider: "we-mp-rss" },
      },
    });
  });
...
Run: `pnpm test src/app/api/settings/integrations/we-mp-rss/__tests__/route.test.ts`
...
```bash
git add src/app/api/settings/integrations/we-mp-rss/__tests__/route.test.ts
git commit -m "test(we-mp-rss): add failing permission tests for settings route lockdown"
```
...
**Files:**
- Modify: `src/app/api/settings/integrations/we-mp-rss/route.ts`（4 个 handler）

- [ ] **Step 1: 修改 import，把 `requireVerifiedUser` 换成 `requireAdmin`**

打开 `src/app/api/settings/integrations/we-mp-rss/route.ts`，把第 3 行：
...
```bash
git add src/app/api/settings/integrations/we-mp-rss/route.ts
git commit -m "feat(we-mp-rss): lock down settings route to admin only"
```
...
**Files:**
- Create: `src/app/api/settings/integrations/we-mp-rss/test/__tests__/route.test.ts`
...
function makeRequest(cookie: string | null, body?: unknown): NextRequest {
  ...
  return new NextRequest(
    "http://localhost/api/settings/integrations/we-mp-rss/test",
    init
  );
}
...
Run: `pnpm test src/app/api/settings/integrations/we-mp-rss/test/__tests__/route.test.ts`
...
```bash
git add src/app/api/settings/integrations/we-mp-rss/test/__tests__/route.test.ts
git commit -m "test(we-mp-rss): add failing permission tests for test-connection route"
```
...
**Files:**
- Modify: `src/app/api/settings/integrations/we-mp-rss/test/route.ts`
...
```bash
git add src/app/api/settings/integrations/we-mp-rss/test/route.ts
git commit -m "feat(we-mp-rss): lock down test-connection route to admin only"
```
...
确认第 101 行是 `{isVerifiedUser && (` 包裹的「外部集成」卡片（href=`/settings/integrations`，描述含「we-mp-rss」）。
...
## Task 7: e2e — 扩充 `api-security.spec.ts` 覆盖 we-mp-rss 接口
...
- [ ] **Step 3: 在 `e2e/api-security.spec.ts` 追加 we-mp-rss 权限用例**
...
test.describe('we-mp-rss 接口权限（P1 收紧）', () => {
  test('VERIFIED_USER 调 GET settings/we-mp-rss → 403', async ({ request }) => {
    await loginAsVerifiedUserAPI(request);
    const res = await request.get('/api/settings/integrations/we-mp-rss');
    expect(res.status()).toBe(403);
  });

  test('VERIFIED_USER 调 POST settings/we-mp-rss → 403', async ({ request }) => {
    await loginAsVerifiedUserAPI(request);
    const res = await request.post('/api/settings/integrations/we-mp-rss', {
      data: { baseUrl: 'http://localhost:8001' },
    });
    expect(res.status()).toBe(403);
  });

  test('VERIFIED_USER 调 POST settings/we-mp-rss/test → 403', async ({ request }) => {
    await loginAsVerifiedUserAPI(request);
    const res = await request.post('/api/settings/integrations/we-mp-rss/test', {
      data: { baseUrl: 'http://localhost:8001' },
    });
    expect(res.status()).toBe(403);
  });

  test('未认证调 GET settings/we-mp-rss → 401', async ({ request }) => {
    const res = await request.get('/api/settings/integrations/we-mp-rss');
    expect(res.status()).toBe(401);
  });

  test('ADMIN 调 GET settings/we-mp-rss → 200', async ({ request }) => {
...
git commit -m "test(e2e): verify we-mp-rss settings routes reject verified users"
```
...
**Files:**
- Create: `docs/handoff/P1-we-mp-rss-permission-handoff.md`
...
- `pnpm lint` → 0 error
- `pnpm test` → 全绿（原有用例 + 新增 10 个 we-mp-rss 单测）
- `pnpm build` → 成功
- `pnpm exec playwright test` → 全绿（含 api-security 新增 5 个用例；如果某个用例因外部 we-mp-rss 服务未起而失败，**仅限 test-connection 类**，记录在交接文档「风控/外部依赖」章节，不算阻塞）

- [ ] **Step 2: 新建交接文档 `docs/handoff/P1-we-mp-rss-permission-handoff.md`**
...
## 风控/外部依赖
无外部依赖失败。test-connection 接口依赖真实 we-mp-rss 服务，但 e2e 用 mock 或不依赖服务可达性，未触发风控。

## 给下一个 PR 的注意事项
- P2 会改 schema，与本 PR 无冲突。
- 管理员后台 `/admin/integrations/we-mp-rss` 保持不动，本 PR 未触碰。
...
```bash
git add docs/handoff/P1-we-mp-rss-permission-handoff.md
git commit -m "docs(handoff): P1 we-mp-rss permission lockdown"
```
...
1. **VERIFIED_USER 登录后**：
   ...
   - 调用 `GET/POST/DELETE/PUT /api/settings/integrations/we-mp-rss` → 403 ✅
   - 调用 `POST /api/settings/integrations/we-mp-rss/test` → 403 ✅
2. **ADMIN 登录后**：
...
3. **匿名（未登录）**：
   - 5 个接口 → 401 ✅
4. **管理员后台 `/admin/integrations/we-mp-rss`**：行为不变 ✅（未触碰）
5. **`/api/integrations/we-mp-rss/**`**（管理员同步/采集接口）：行为不变 ✅（本就是 requireAdmin）
...
**Spec coverage：**
- 需求 3.1「we-mp-rss 仅管理员」→ Task 1-7 全覆盖（5 个 API + 入口 + 守卫 + 单测 + e2e）✅
- 需求 6.1「禁用普通用户 we-mp-rss 配置接口」→ Task 1-4 覆盖 ✅
- 文档列出的检查点 `/integrations/we-mp-rss` 普通用户入口 → Task 5-6 覆盖 ✅
