# 交接报告：we-mp-rss 公众号同步脚本

> 日期：2026-06-11（更新）
> 脚本：`scripts/deploy/sync-we-mp-rss-feeds.sh`

---

## 功能

对比本地 we-mp-rss SQLite 数据库和线上 we-mp-rss 实例的公众号列表，支持自动同步缺失的 feed。

## 使用方法

```bash
# 预览差异（dry-run，不执行同步）
bash scripts/deploy/sync-we-mp-rss-feeds.sh

# 自动同步（通过 tRPC API 添加缺失的 feed）
AUTH_CODE='管理密码' bash scripts/deploy/sync-we-mp-rss-feeds.sh --apply

# 通过 SSH 在远程服务器执行
AUTH_CODE='管理密码' SSH_PASS='服务器密码' bash scripts/deploy/sync-we-mp-rss-feeds.sh --apply --remote
```
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `AUTH_CODE` | we-mp-rss 管理密码（--apply 模式必须） | — |
| `REMOTE_HOST` | 线上 we-mp-rss 地址 | `47.119.182.210` |
| `REMOTE_PORT` | 线上 we-mp-rss 端口 | `4000` |
...
we-mp-rss v2.6.1 使用 tRPC 协议暴露 API（之前误以为无公开 API，实际可用）：
...
通过分析 we-mp-rss 前端 JS bundle（`/dash/assets/index-*.js`）发现：
...
- 本地数据库：`infra/wechat-rss/we-mp-rss/data/we-mp-rss.db` → `feeds` 表
