...
### 测试
- 新建 `src/app/api/settings/integrations/wewe-rss/__tests__/route.test.ts`（7 用例）
- 新建 `src/app/api/settings/integrations/wewe-rss/test/__tests__/route.test.ts`（3 用例）
...
- `pnpm test`（vitest）：**280/280 passed**（38 文件，含 P1 新增 10 个 wewe-rss 权限单测）
- `pnpm build`：**成功**
- `pnpm exec playwright test e2e/api-security.spec.ts -g "WeWe RSS"`：**6/6 passed**
  - 5 个匿名 → 401
...
- `d6835ab` test(wewe-rss): add failing permission tests for settings route lockdown
- `82f286f` test(wewe-rss): fix auth mock to expose requireVerifiedUser
- `82efc0e` feat(wewe-rss): lock down settings route to admin only
- `95eb30d` test(wewe-rss): add failing permission tests for test-connection route
- `f55653d` feat(wewe-rss): lock down test-connection route to admin only
...
- `d81df41` test(e2e): verify wewe-rss settings routes anon-401 and admin-200

## 给后续 PR 的注意事项
- `/api/integrations/wewe-rss/**`（管理员同步/采集接口）本就是 `requireAdmin`，P1 未触碰，保持不变。
- 管理员后台 `/admin/integrations/wewe-rss` 页面保持不动。
