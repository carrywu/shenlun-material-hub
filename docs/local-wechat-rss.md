...
| 本地 WeWe RSS | 是 | RSS 本身不需要 Key | 推荐短期 POC |
...
详见 [`infra/wechat-rss/wewe-rss/README.md`](../infra/wechat-rss/wewe-rss/README.md)。

### 接入方式

1. 启动 WeWe RSS 后，获取公众号的 RSS 地址
...
| 维度 | WeWe RSS | we-mp-rss |
|---|---|---|
| 维护状态 | 已归档 (2026-05) | 活跃维护 |
| Stars | 9.5k | 3.3k |
| 登录方式 | 微信读书扫码 | 微信扫码 |
| 数据库 | MySQL / SQLite | SQLite |
| Docker 镜像 | `cooderl/wewe-rss-sqlite` | `ghcr.io/rachelos/we-mp-rss` |
| 默认端口 | 4000 | 8001 |
...
如果 WeWe RSS 因归档导致功能失效，we-mp-rss 是最合适的替代方案。后续可添加 `infra/wechat-rss/we-mp-rss/` 部署示例。
