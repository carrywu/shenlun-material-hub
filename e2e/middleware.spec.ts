...
test.describe("Middleware redirect", () => {
  ...
  test("公开页面：/search 无需登录可访问", async ({ page }, testInfo) => {
    const guard = attachConsoleGuard(page);
  ...
  test("公开页面：/cards 无需登录可访问", async ({ page }, testInfo) => {
    const guard = attachConsoleGuard(page);
  ...
  test("受保护 API：/api/content-items 返回 401", async ({ request }) => {
    const res = await request.get("/api/content-items");
    expect(res.status()).toBe(401);
  });

  test("受保护 API：/api/search 返回 401", async ({ request }) => {
  ...
  test("公开页面无 console error", async ({ page }) => {
    ...
    for (const url of ["/articles", "/explore", "/search", "/discover"]) {
      await page.goto(url);
      await page.waitForTimeout(1000);
    }

    // Filter out known acceptable errors (401 for public search is expected)
