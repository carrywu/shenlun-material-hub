  ...
  async collect(source: {
    ...
    for (const article of articles.slice(0, 5)) {
      try {
        ...
        const fullText =
          $d(".rm_txt_con").text().trim() ||
          $d("#rwb_zw").text().trim() ||
          $d(".text_con").text().trim() ||
          $d(".article_content").text().trim() ||
          $d("div[class*='content']").first().text().trim();

        // 提取作者
        const author =
          $d(".author").text().trim() ||
          $d(".rm_txt_con .fl a").first().text().trim() ||
          undefined;
