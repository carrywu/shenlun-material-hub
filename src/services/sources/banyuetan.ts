...
const SOURCE_NAME = "半月谈";
const LIST_URL = "http://www.banyuetan.org/";
const rateLimiter = new RateLimiter(3000);

export const banyuetanCollector: ArticleCollector = {
  ...
  async collect(): Promise<CollectedArticle[]> {
    ...
    try {
      ...
      $("a").each((_, el) => {
        ...
        if (
          text.length >= 4 &&
          (href.includes("banyuetan.org") || href.startsWith("/")) &&
      ...
      for (const url of targetLinks) {
        try {
          await rateLimiter.wait();
          const article = await fetchBanyuetanDetail(url);
          if (article) {
...
async function fetchBanyuetanDetail(
  url: string
  ...
  try {
    ...
    let content = "";
    const contentEl = $(
      ".article-content, .content, .text_con, #zoom, .TRS_Editor"
    ).first();
    if (contentEl.length) {
      content = contentEl
        .find("p")
    ...
    const timeText = $(".article-info, .pub_date, time").first().text() ?? "";
