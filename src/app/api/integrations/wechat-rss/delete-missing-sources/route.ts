...
// POST /api/integrations/wechat-rss/delete-missing-sources — 删除 we-mp-rss 中已不存在的来源
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 安全检查：只允许删除 provider=we-mp-rss 的来源
    ...
    if (sources.length === 0) {
      return NextResponse.json({
        success: true,
        message: "没有可删除的 we-mp-rss 来源",
        deleted: 0,
        contentItemsDeleted: 0,
      });
    }

    // 检查是否有非 we-mp-rss 的来源被误传
    ...
    return NextResponse.json({
      success: true,
      message: `已删除 ${sources.length} 个 we-mp-rss 来源`,
