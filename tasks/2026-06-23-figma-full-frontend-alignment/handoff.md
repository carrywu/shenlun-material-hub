...
- Added a dedicated no-DB Playwright config/spec for the public authentication pages and saved mobile/tablet/desktop light/dark screenshots.
...
- A no-DB browser harness covers `/login`, `/register`, and `/admin/login` across 390 / 768 / 1440 and light / dark. It does not cover `/articles/[id]` because middleware correctly redirects protected article pages to `/login` without a valid DB-backed session.
...
- `pnpm exec playwright test e2e/figma-approved-no-db.spec.ts --config e2e/figma-no-db.playwright.config.ts`: passed, 18 auth-page no-DB tests across 390 / 768 / 1440 and light / dark.
- `pnpm exec eslint e2e/figma-approved-no-db.spec.ts e2e/figma-no-db.playwright.config.ts src/app/globals.css src/app/login/page.tsx src/app/register/page.tsx`: passed for configured TS/TSX targets; CSS file is ignored by ESLint config.
